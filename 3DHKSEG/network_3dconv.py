...
class hk3dlayer_srh(nn.Module):
    ...
    def forward(self,x):
        ...
        O2 = [F.conv3d(x, torch.mul(self.W, self.mask_1), stride=1, padding=(4, 4, 4)),
        ...
        x = self.a[0] * O2[0].squeeze(1) + self.a[1] * O2[1].squeeze(1) + \
            self.a[2] * O2[2].squeeze(1) + self.a[3] * O2[3].squeeze(1)

        x = self.relu(self.gns(x))
...
class hk3dlayer_evl(nn.Module):
    ...
    def forward(self,x):
        ...
        x = self.relu(self.gnm(O1[self.idx].squeeze(1)))
