...
class hk3dlayer_srh(nn.Module):
    def __init__(self, in_channel):
        ...
        self.W = nn.Parameter(torch.randn(1, 1, 9, 9, 9))
        self.relu = nn.ReLU(inplace=True)
        #self.bns1 = nn.ModuleList()
        self.bns = nn.BatchNorm3d(1)
    ...
    def forward(self,x):
        ...
        x = self.bn1(self.conv1(x)) #b, c, h, w
        ...
        self.a[0] = torch.sum(torch.mul(self.W, self.mask_1), (4, 3, 2, 1, 0)) / (
                torch.sum(self.mask_1, (4, 3, 2, 1, 0)) + 1e-10)
        self.a[1] = torch.sum(W_circle1, (4, 3, 2, 1, 0)) / (
                torch.sum(self.mask_2 - self.mask_1, (4, 3, 2, 1, 0)) + 1e-10)
        self.a[2] = torch.sum(W_circle2, (4, 3, 2, 1, 0)) / (
                torch.sum(self.mask_3 - self.mask_2, (4, 3, 2, 1, 0)) + 1e-10)
        self.a[3] = torch.sum(W_circle3, (4, 3, 2, 1, 0)) / (
                torch.sum(self.mask_4 - self.mask_3, (4, 3, 2, 1, 0)) + 1e-10)

        self.a = F.softmax(self.a, dim=0)

        O = [F.conv3d(x, torch.mul(self.W, self.mask_1), stride=1, padding=(4, 4, 4)),
              F.conv3d(x, torch.mul(self.W, self.mask_2), stride=1, padding=(4, 4, 4)),
              F.conv3d(x, torch.mul(self.W, self.mask_3), stride=1, padding=(4, 4, 4)),
              F.conv3d(x, torch.mul(self.W, self.mask_4), stride=1, padding=(4, 4, 4))]
...
class hk3dcls_srh(nn.Module):
    def __init__(self, spec_band, num_classes, block_num, layer_num, init_weights=True):
        ...
        self.fc = nn.Linear(out_channel, num_classes).float()
...
class hk3dlayer_evl(nn.Module):
    def __init__(self, in_channel, layer_arch):
        ...
        #self.bnm1 = nn.BatchNorm3d(in_channel // 4)
        self.bnm2 = nn.BatchNorm3d(1)
    ...
    def forward(self,x):
        ...
        O1=[F.conv3d(x, torch.mul(self.W, self.mask_1), stride=1, padding=(4,4,4)),
           F.conv3d(x, torch.mul(self.W, self.mask_2), stride=1, padding=(4,4,4)),
           F.conv3d(x, torch.mul(self.W, self.mask_3), stride=1, padding=(4,4,4)),
           F.conv3d(x, torch.mul(self.W, self.mask_4), stride=1, padding=(4,4,4))]

        x = self.relu(self.bnm2(O1[self.idx]))
