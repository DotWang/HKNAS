...
class hk3dlayer_srh(nn.Module):
    ...
    def forward(self,x):
        ...
        O2 = [F.conv2d(x, torch.mul(self.W2, self.mask_21), stride=1, padding=(4, 4), groups=cc),
              F.conv2d(x, torch.mul(self.W2, self.mask_22), stride=1, padding=(4, 4), groups=cc),
              F.conv2d(x, torch.mul(self.W2, self.mask_23), stride=1, padding=(4, 4), groups=cc),
              F.conv2d(x, torch.mul(self.W2, self.mask_24), stride=1, padding=(4, 4), groups=cc)]

        x = self.a2[0] * O2[0] + self.a2[1] * O2[1] + \
...
class hk3dlayer_evl(nn.Module):
    ...
    def forward(self,x):
        ...
        x = self.relu(self.bnm2(O1[self.idx1]))
