...
class product():
    ...
    def generation_num(self, labeled_data, rows_num, All_data):
        ...
        return list(map(int, rows_num)), list(map(int, trn_num)), list(map(int, val_num)), list(
            map(int, tes_num)), list(map(int, pre_num))
    ...
    def production_data_trn(self, rows_num, trn_num, half_s, image_3d_mat):
        ...
        val_num = list(set(rows_num) - set(neibour_num))  # 防止数据偷看
